...
BASEFLAGS="-C link-arg=-Ttarget/tab/$1/riscv32_layout.ld -C relocation-model=static -C link-arg=--emit-relocs -D warnings"
...
#RUSTFLAGS=$FIRST_FLAGS cargo rustc --verbose --release --target=riscv32imac-unknown-none-elf --example "$1"
	...
	elf2tab --verbose -n "$NAME" -o "$tab_file_name" "$elf_file_name" --stack 2048 --app-heap 512 --kernel-heap 1024 &> /tmp/bad
}
